...
MULTI_SECTOR_HEADER_SIGNATURES_SUPPORTED = [b'FILE', b'BAAD', b'CHKD']
...
FlagList = {
    ...
    0x00001000: 'OFFLINE',
    0x00002000: 'NOTINDEXED',
    0x00004000: 'ENCRYPTED',
    0x10000000: 'DIRECTORY',
    0x20000000: 'INDEXVIEW'
}

def ResolveFlagCodes(FlagCodes):
    flags_left = FlagCodes

    str_list = []
    for flag in sorted(FlagList.keys()):
        if FlagCodes & flag > 0:
            str_list.append(FlagList[flag])
            flags_left -= FlagCodes & flag
...
class MasterFileTableParser(object):
    ...
    def __init__(self, file_object, do_first_pass=True):
        ...
        file_record_segment_size_bytes = self.file_object.read(4) # 00 04 00 00 -> 1024
        ...
        if self.file_record_segment_size not in [1024, 4096]:
...
class FileRecordSegment(object):
    ...
    def __init__(self, file_record_segment_buf, apply_update_sequence_array=True):
        ...
        if len(file_record_segment_buf) not in [1024, 4096]:
            raise FileRecordSegmentException(
    ...
    def validate_file_record_segment_header(self):
        ...
        if file_record_segment_size not in [1024, 4096]:
            raise FileRecordSegmentException(
    ...
    def is_bad(self):
        ...
        return struct.unpack('<4s', self.frs_data[: 4])[0] != b'FILE'
    ...
    def attributes(self):
        ...
        while pos < self.get_first_free_byte_offset():
            ...
            if len(attribute_record_partial_header) == 16:
                type_code, record_length, form_code, name_length, name_offset, flags, instance = UnpackAttributeRecordPartialHeader(
                    attribute_record_partial_header)
                        ...
                        while curr_pos > 0:
                            ...
                            if byte_1 in printable and byte_2 == 0:
                                ...
                                file_name_reversed += chr(byte_1)
                                    ...
                                    file_name_reversed) >= 5:  # Stop if a non-ASCII character is encountered and we have a plausible file name.
                                break
