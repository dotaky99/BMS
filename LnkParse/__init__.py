...
class lnk_file(object):
	...
	def clean_line(rstring):
		...
		return ''.join(chr(i) for i in rstring) #chr 아스키코드를 문자로 변환하는 함수 -> 한글출력 불가
	...
	def parse_link_information(self, index):
		...
		if self.loc_information['link_info_flags'] & 0x0001:
			...
			if self.loc_information['link_info_header_size'] >= 36:
				...
				test = self.read_string(local_index)

				self.loc_information['local_base_path'] = test.encode("ISO-8859-1").decode('euc-kr')
			...
			if self.loc_information['location_info']['volume_label_offset'] != 20:
				...
				test = self.read_string(local_index)
				self.loc_information['location_info']['volume_label'] = test.encode('ISO-8859-1').decode('euc-kr')
			else:
	...
	def parse_string_data(self, index):
		...
		if self.linkFlag['HasWorkingDir']:
			...
			self.data['working_directory'] = self.read_string(index)
	...
	def parse_knownFolder_block(self, index, size):
		...
		self.extraBlocks['KNOWN_FOLDER_LOCATION_BLOCK']['known_folder_guid'] = self.indata[index + 8: index + 24].hex()

	def print_lnk_file(self):
		print('[Shell Link Header]')
		print('\tFile Attributes : %s' % (self.format_fileFlags()))
		print('')
		try:
		...
		print('\tdrive_type: ' +self.loc_information['location_info']['drive_type'])
		print('\tdrive_serial_number: ' +self.loc_information['location_info']['drive_serial_number'])
		print('\tlocal_base_path: ' + self.loc_information['local_base_path'])
	...
	def read_string(self, index):
		...
		while self.indata[index] != 0x00:
			result += chr(self.indata[index]) #아스키코드로 변환 후 문자열로 이어붙이는 단계
			index += 1
		return result
