...
class Tab3():
    def set_tab3(self):
        global timeline, checkbox1_1, checkbox1_2
        # tab3 구성
        self.tab3.layout = QGridLayout(self)

        self.box1 = QVBoxLayout()
        checkbox1_1 = QCheckBox("MFT 생성")
        checkbox1_2 = QCheckBox("계정 생성")

        self.checkbox1_3 = QCheckBox("Windows 설치")
        self.checkbox1_4 = QCheckBox("Windows 업데이트")
        self.checkbox1_5 = QCheckBox("시스템 On/Off")
        self.box1.addWidget(checkbox1_1)
        self.box1.addWidget(checkbox1_2)
        ...
        self.timebutton.clicked.connect(Tab3.set_timeline)
        ...
        timeline = QTableWidget(self)
        timeline.setSortingEnabled(True)
        timeline.setColumnCount(4)
        headers = ["시간", "행위", "세부 사항", "경로"]
        timeline.setHorizontalHeaderLabels(headers)

        self.tab3.layout.addLayout(self.box3, 0, 0)
        self.tab3.layout.addLayout(self.box4, 1, 0)
        self.tab3.layout.addWidget(timeline, 2, 0)
        self.tab3.setLayout(self.tab3.layout)

        # self.checkbox1_1.stateChanged.connect(Tab3.set_timeline)

    def set_timeline(self):
        # self.datetime1 = self.input_datetime1.dateTime()
        # self.datetime2 = self.input_datetime2.dateTime()
        global timeline_count
        timeline.clearContents()
    ...
    def timeline_data1_2(self):
        ...
        rows = cur.fetchall()
        conn.close()
        count = timeline_count + len(rows)
        timeline.setRowCount(count)
        password_exists = []
        for i in range(len(rows)):
            created_on, last_password_change_time, account_name, RID_int = rows[i]
